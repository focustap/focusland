...
The multiplayer party card game lives at `/arcade/campfire-cards`. Its card pack content is stored in `src/lib/campfire-cards/packs.ts`.
...
Keep pack ids stable because rooms store enabled pack ids in realtime state. Bundled Campfire Cards art/audio sources and card import notes are listed in `public/assets/campfire-cards/CREDITS.md`.

To generate the default-off Imported Party Pack from an original/permissively licensed CSV, run:

```sh
node scripts/import-campfire-cards.mjs path/to/cards.csv
```

The importer accepts either `type,text` style columns or cells prefixed with `Prompt`, `Response`, or `Answer`. It keeps only one-answer prompts, preserves `______`, filters `PICK 2`/`PICK 3`/draw-multiple prompts, and deduplicates normalized text before writing `src/lib/campfire-cards/importedPartyPack.ts`.
